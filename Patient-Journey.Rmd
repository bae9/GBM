...
stripchart(combo$dropoutsum.Sum / combo$arms.Enrollment ~ combo$arms..Universal.IV., add=TRUE, col='RED', method='jitter')

#add text
text(x=0.08, y=1, labels=toString(round(bxp$stats[3,1] * 100, digits=2)))
text(x=0.2, y=2, labels=toString(round(bxp$stats[3,2] * 100, digits=2)))
title(paste("% Patient Dropout rate by use of Universal IV \n n=", sum(combo$arms.Enrollment), sep=""))
...
stripchart(combo$dropoutsum.Sum / combo$arms.Enrollment ~ combo$arms.Recurrent, add=TRUE, col='RED', method="jitter")

title(paste("% Patient Dropout rate by Recurrence \n n=", sum(df_arm$arms.Enrollment), sep=""))
text(x=0.15, y=1, labels=toString(round(bp$stats[3,1] * 100, digits=2)))
text(x=0.15, y=2, labels=toString(round(bp$stats[3,2] * 100, digits=2)))
```
